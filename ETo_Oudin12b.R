...
#
# Oudin ETo calibration – global "a" + 12 monthly "b" parameters (12b) with train/test split, daily time steps
#
# Model form (daily):
#   ETo_mod = pmax(0, a_all * Ra * (Tavg + b_m))
#   where:
#     - Tavg = 0.5*(Tmax + Tmin)
#     - a_all is a single calibration parameter applied to all months (global)
#     - b_m is a calibration parameter for calendar month m (m = 1..12)
#     - Ra is extraterrestrial radiation (MJ m^-2 d^-1)
#
# Parameters and bounds:
#   a_all ∈ [0.0001, 0.02] (calibrated)
...
#     Level 2: Differential Evolution on 13 parameters (a_all, b01..b12) to maximize 
#              KGE on daily TRAINING data. The first seven members of the initial population are seeded 
#              with the seven Level 1 results (each as c(a_global_L1, b_precalc_01..12)).
#     Level 3: nlminb algorithm refines all 13 parameters starting from Level 2 best solution,
...
#     the final a_all (8 decimals); b01..b12 (8 decimals); TRAIN and TEST metrics such as 
#     KGE, r, mean_monthly, cv_obs, cv_sim, cv_ratio; boundary flags for
#     a_all and b parameters; and diagnostic metrics. Additionally includes quadrangle-level optimization
...
out_dir <- "G:/EvapModels/ETo_Oudin12b/Training_data/"
...
digits_par    <- 8   # parameters a_all, b01..b12, a_global_L1, b01_precalc..b12_precalc
...
.keep_names <- c(ls(all.names = TRUE), ".keep_names")
...
for (qid_num in qnum) {
  ...
  out_diag <- file.path(out_dir, sprintf("Oudin12b_KGE_calibration_1979-2018_Q%s.tsv", qid))
  out_mod  <- file.path(out_dir, sprintf("Oudin12b_KGE_calibrated.modeled_ETo.1979-2018.Q%s.txt", qid))
  out_ra   <- file.path(out_dir, sprintf("Oudin12b_KGE_calibrated.Ra.1979-2018.Q%s.txt", qid))
  ...
  par_hat_quad <- rep(NA_real_, 13)
  names(par_hat_quad) <- c("a_all", sprintf("b%02d", 1:12))
  quad_KGE <- NA_real_
  quad_r <- NA_real_
  ...
  quad_L2_results <- list(params = rep(NA_real_, 13), KGE = NA_real_, 
                          r = NA_real_, mean_monthly = NA_real_,
                          cv_obs = NA_real_, cv_sim = NA_real_, cv_ratio = NA_real_)
  quad_L3_results <- list(params = rep(NA_real_, 13), KGE = NA_real_, 
                          r = NA_real_, mean_monthly = NA_real_,
                          cv_obs = NA_real_, cv_sim = NA_real_, cv_ratio = NA_real_)
  ...
  if (has_na_quad) {
    ...
    for (idx in 1:7) {
      ...
      if (!anyNA(b_precalc_quad)) {
        ...
        if (!inherits(opt_quad_L1, "try-error") && is.finite(opt_quad_L1$maximum)) {
          ...
          par0_quad_L1_list[[idx]] <- c(a_L1_quad, b_precalc_quad)
    ...
    if (all(sapply(par0_quad_L1_list, is.null))) {
      ...
      lower_vec_24 <- c(a_all = lower_a_vec, lower_b_vec)  # name kept for minimal code churn
      upper_vec_24 <- c(a_all = upper_a_vec, upper_b_vec)
      
      kge_train_quad_24 <- function(par) {
        a <- par[1]
        b <- par[2:13]
        b_day <- b[month_ix_train]
        sim_daily <- pmax(0, a * Ra_quad_train * (Tavg_quad_train + b_day))
        -(kge_objfunc(obs_quad_train, sim_daily, cv_obs_quad_train, month_ix_train, mean_obs_monthly_quad))
      }
      ...
      NP <- 130
      initialpop_quad <- matrix(NA_real_, nrow = NP, ncol = 13)
      # Column 1 is a_all
      initialpop_quad[, 1] <- runif(NP, min = 0.001, max = 0.02)
      # Columns 2:13 are b parameters
      for (kk in 2:13) {
      ...
      if (!inherits(de_fit_quad, "try-error") && is.finite(de_fit_quad$optim$bestval)) {
        ...
        a_L2 <- par0_quad_24[1]
        b_L2 <- par0_quad_24[2:13]
        b_day_L2 <- b_L2[month_ix_train]
        sim_daily_L2 <- pmax(0, a_L2 * Ra_quad_train * (Tavg_quad_train + b_day_L2))
      ...
      if (inherits(fit_nb_quad, "try-error") || !is.finite(fit_nb_quad$objective) || any(is.na(fit_nb_quad$par))) {
        ...
        a_L3 <- par_hat_quad[1]
        b_L3 <- par_hat_quad[2:13]
        b_day_L3 <- b_L3[month_ix_train]
        sim_daily_L3 <- pmax(0, a_L3 * Ra_quad_train * (Tavg_quad_train + b_day_L3))
      ...
      a_day_quad <- rep(par_hat_quad[1], length(month_ix_train))
      b_day_quad <- par_hat_quad[2:13][month_ix_train]
  ...
  for (j in 1:n_cells) {
    ...
    if (skip_all_qdgcs) {
      diag_list[[j]] <- do.call(
        ...
        c(
          list(qdgc_id = qid_j),
          # a_all
          setNames(as.list(rep(NA_real_, 1)), a_names),
          ...
          # boundary flags
          setNames(as.list(rep(NA, 1)), paste0(a_names, "_at_boundary")),
          ...
          setNames(as.list(rep(NA_real_, 1)), a_quad_names),
    ...
    if (has_na_full) {
      diag_list[[j]] <- do.call(
        ...
        c(
          ...
          setNames(as.list(par_hat_quad[1]), a_quad_names),
          setNames(as.list(par_hat_quad[2:13]), b_quad_names),
    ...
    # optimizing all 13 parameters (a_all, b01..b12). The first member of the initial population
    ...
    lower_b_vec <- pmax(-30.0, par_hat_quad[2:13] - 2.5)
    upper_b_vec <- pmin(60.0, par_hat_quad[2:13] + 2.5)
    names(lower_b_vec) <- b_names
    names(upper_b_vec) <- b_names
    
    # Bounds for a_all (unchanged)
    lower_a_vec <- lower_a
    upper_a_vec <- upper_a
    
    # Combined bounds for 13 parameters (a_all, b01..b12)
    lower_vec_24 <- c(a_all = lower_a_vec, lower_b_vec)
    upper_vec_24 <- c(a_all = upper_a_vec, upper_b_vec)
    
    kge_train_obj_24par <- function(par) {
      # par: a_all, b01..b12 (13 parameters)
      a <- par[1]
      b <- par[2:13]
      b_day <- b[month_ix_train]  # b for each TRAIN day
      sim_daily <- pmax(0, a * Ra_train * (Tavg_train + b_day))
    ...
    NP <- 130  # 13 parameters × 10
    initialpop <- matrix(NA_real_, nrow = NP, ncol = 13)
    # Column 1 is a_all
    initialpop[, 1] <- runif(NP, min = 0.001, max = 0.02)
    # Columns 2:13 are b parameters
    for (kk in 2:13) {
    ...
    for (seed_idx in 2:5) {
      noise_factor <- runif(13, min = -0.1, max = 0.1)
      ...
      lower_all <- as.numeric(lower_vec_24); upper_all <- as.numeric(upper_vec_24)
      perturbed_params <- pmax(lower_all, pmin(upper_all, perturbed_params))
    ...
    # -----------------------
    # Level 3 (nlminb): Local refinement of all 13 parameters; uses KGE calculated from daily
    ...
    if (inherits(fit_nb, "try-error") || !is.finite(fit_nb$objective) || any(is.na(fit_nb$par))) {
      a_hat <- setNames(as.list(par0_24[1]), a_names)
      b_hat <- setNames(as.list(par0_24[2:13]), b_names)
      note_txt <- "Level 3 failed; using Level 2 solution"
    } else {
      a_hat <- setNames(as.list(fit_nb$par[1]), a_names)
      b_hat <- setNames(as.list(fit_nb$par[2:13]), b_names)
      note_txt <- ""
    }
    ...
    # -----------------------
    a_all_days <- unlist(a_hat)[1]
    b_day_all <- unlist(b_hat)[month_ix]
    sim_all <- pmax(0, a_all_days * Ra_j * (Tavg_j + b_day_all))
    sim_mat_all[, j] <- sim_all
    
    # Collect diagnostics
    # Column order: qdgc_id, a_all, b01..b12, KGE metrics, boundary flags (a then b), diagnostics,
    # quadrangle-level parameters (a_all_quad, b01_quad..b12_quad)
    ...
    diag_row[, (a_quad_names) := par_hat_quad[1]]
    for (m in 1:12) {
      diag_row[, (b_quad_names[m]) := par_hat_quad[m + 1]]
    }
  ...
  for (row_idx in 1:7) {
    ...
    if (!is.null(L1_res$params) && !all(is.na(L1_res$params))) {
      # a_all_quad
      set(diag_dt, i = row_idx, j = a_quad_names, value = L1_res$params[1])
      # b01..b12_quad
      for (m in 1:12) {
        set(diag_dt, i = row_idx, j = b_quad_names[m], value = L1_res$params[m + 1])
      }
  ...
  if (!all(is.na(quad_L2_results$params))) {
    set(diag_dt, i = 8L, j = a_quad_names, value = quad_L2_results$params[1])
    for (m in 1:12) {
      set(diag_dt, i = 8L, j = b_quad_names[m], value = quad_L2_results$params[m + 1])
    }
  ...
  if (!all(is.na(quad_L3_results$params))) {
    set(diag_dt, i = 9L, j = a_quad_names, value = quad_L3_results$params[1])
    for (m in 1:12) {
      set(diag_dt, i = 9L, j = b_quad_names[m], value = quad_L3_results$params[m + 1])
    }
  ...
  for (row_idx in 10:16) {
    set(diag_dt, i = row_idx, j = a_quad_names, value = NA_real_)
    for (m in 1:12) {
  ...
  message("Diagnostics (Oudin 12b; train/test with daily KGE): ", out_diag)
  message("Modeled ETo (Oudin 12b; 1979-2018, daily time step): ", out_mod)
